...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        internalStoragePhotoAdapter = InternalStoragePhotoAdapter {
            lifecycleScope.launch {
                ...
                if (isDeletionSuccessful) {
                    ...
                    Toast.makeText(this@MainActivity, "Foto excluída com sucesso", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(this@MainActivity, "Falha ao excluir foto", Toast.LENGTH_SHORT).show()
                }
            }
        ...
        permissionsLauncher = registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { permissions ->
            ...
            if (readPermissionGranted) {
                ...
                Toast.makeText(this, "Não é possível ler arquivos sem permissão.", Toast.LENGTH_LONG).show()
            }
        ...
        intentSenderLauncher = registerForActivityResult(ActivityResultContracts.StartIntentSenderForResult()) {
            if (it.resultCode == RESULT_OK) {
                ...
                Toast.makeText(this@MainActivity, "Falha ao excluir a foto", Toast.LENGTH_SHORT).show()
            }
        }
        ...
        val takePhoto = registerForActivityResult(ActivityResultContracts.TakePicturePreview()) {
            lifecycleScope.launch {
                ...
                if (isSavedSuccessfully) {
                    Toast.makeText(this@MainActivity, "Foto salva com sucesso", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(this@MainActivity, "Falha ao salvar a foto", Toast.LENGTH_SHORT).show()
                }
            }
        ...
        setupInternalStorageRecycleView()
        loadPhotosFromInternalStorageIntoRecycleView()
        loadPhotosFromExternalStorageIntoRecycleView()
                ...
                contentResolver.insert(imageCollection, contentValues)?.also { uri ->
                    contentResolver.openOutputStream(uri).use { outputStream ->
                        if (!bmp.compress(Bitmap.CompressFormat.JPEG, 95, outputStream)) {
                            throw IOException("Não foi possível salvar o bitmap")
                        }
                    }
                } ?: throw IOException("Não foi possível criar a entrada do MediaStore")
                true
            } catch (e: IOException) {
                e.printStackTrace()
                false
    ...
    private fun setupInternalStorageRecycleView() = binding.rvPrivatePhotos.apply {
            ...
            try {
                openFileOutput("$filename.jpg", Context.MODE_PRIVATE).use { stream ->
                    if (!bmp.compress(Bitmap.CompressFormat.JPEG, 95, stream)) {
                        throw IOException("Não foi possível salvar a foto.")
                    }
                    true // Retornar true se a operação for bem-sucedida
                }
            } catch (e: IOException) {
                e.printStackTrace()
                false // Retornar false se ocorrer uma exceção
            }
        }
    }
